...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                TriggerClientEvent('QBCore:Notify', source, 'Time is now frozen.')
            else
                TriggerClientEvent('QBCore:Notify', source, 'Time is no longer frozen.')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Error: You are not allowed to use this command.', 'error')
        end
    else
        freezeTime = not freezeTime
        if freezeTime then
...
function NextWeatherStage()
    if CurrentWeather == "CLOUDS" or CurrentWeather == "SUNNY"  then
    ...
    elseif CurrentWeather == "HIGHPRESSURE" or CurrentWeather == "OVERCAST" or CurrentWeather == "OVERCASTDARK" then
